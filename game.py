...
class Pong(object):
    def __init__(self, screensize):
        ...
        #Define Randomicamente qual direção a bola vai sair
...
class AIPaddle_hor(object):
    ...
    def update(self, Balls, corner_R, corner_L):
        #TESTE
        #Checar se é o paddle de cima ou de baixo
        if self.rect.y < 100:
            for Ball in Balls:
                if Ball.rect.y < 150:        
...
def main():
    ...
    while running:
        ...
        if (time/pygame.time.get_ticks() < 1):
            Balls.append(Pong(screensize)) 
            time += 10000
        #---------------    
